...
class SkiEnv(gym.Env):
    ...
    # Skier
    MASS         = 75.0          # kg
    # Speed — drag coefficients are forces (N·s²/m²), divided by MASS in physics
    BASE_DRAG    = 1.50          # air/snow friction (always present)
    BRAKE_DRAG   = 18.75         # additional drag when braking fully
    TURN_DRAG    = 7.50          # additional drag from carving (proportional to |steer|)
    EDGE_DRAG    = 37.5          # edge braking when perpendicular to fall line
    CARVE_FORCE  = 375.0         # carving force coefficient (N·s/m)
    ...
    def step(self, action):
        ...
        # Drag opposes current velocity (force / mass → acceleration)
        if speed > 0.01:
            drag_ax = -drag_coeff * vx * speed / self.MASS
            drag_ay = -drag_coeff * vy * speed / self.MASS
        else:
            drag_ax = 0.0
            drag_ay = 0.0
        ...
        # Strength proportional to speed (force / mass → acceleration)
        carve_strength = self.CARVE_FORCE * speed / self.MASS
