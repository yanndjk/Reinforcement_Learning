"""
ski_env_legacy.py — V1 environment: torque-based dynamics, 1D action.

Kept for reference and progress reporting. The current version (ski_env.py)
uses heading-based dynamics with a 2D action space.

State (14D):
    [x, y, vx, vy, angle, angular_vel,          # 0-5: physics
     gate1_rel_x, gate1_rel_y,                   # 6-7: next gate
     gate2_rel_x, gate2_rel_y,                   # 8-9: gate after next
     gate_width,                                  # 10: current gate width (normalised)
     lateral_error,                               # 11: vx sign relative to next gate
     gates_passed_ratio,                          # 12: course progress
     difficulty]                                  # 13: current difficulty level
...
class SkiEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        # Gate width normalised to [0, 1] range (2.5 is max, 1.2 is min)
        gate_width_norm = self.gate_width / 2.5

        # Lateral error: how fast we're moving toward/away from next gate
        next_gx = self._next_gate_x(y)
        lateral_offset = next_gx - x
        # positive = vx moving toward gate, negative = moving away
    ...
    def _render_frame(self):
        ...
        if self.screen is None:
            ...
            if self.render_mode == "human":
                ...
                pygame.display.set_caption("Ski RL — Competition Slalom (Legacy v1)")
            else:
