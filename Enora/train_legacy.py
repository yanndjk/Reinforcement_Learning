"""
train_legacy.py — V1 training loop (torque-based dynamics, 1D action).

Kept for reference and progress reporting. Uses ski_env_legacy and ppo_agent_legacy.
All commands work the same as train.py — just run with: python train_legacy.py

Usage:
    python train_legacy.py                    # train from scratch
    python train_legacy.py --render           # train with live rendering (slow)
    python train_legacy.py --eval             # evaluate a saved checkpoint
"""
...
# Local imports — pointing at legacy (v1) modules
import sys
sys.path.insert(0, str(Path(__file__).parent))
from envs.ski_env_legacy import SkiEnv
from agents.ppo_agent_legacy import ActorCritic, RolloutBuffer, ppo_update
